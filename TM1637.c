...
//******************************************
void TM1637_displayRaw(uint8_t BitAddr,int8_t SegData)
{
  //int8_t SegData;
...
void TM1637_displayRaw(const int8_t* SegData)
{
  TM1637_start();          //start signal sent to TM1637 from MCU
  TM1637_writeByte(ADDR_AUTO);//
  TM1637_stop();           //
  ...
  TM1637_writeByte(SegData[0]);
  TM1637_writeByte(SegData[1]);
  TM1637_writeByte(SegData[2]);
  TM1637_writeByte(SegData[3]);
...
void TM1637_displayRaw(int n)
{
  int8_t txtBuff[4];
  for (int i = 3; i >= 0; i--)
  {
     txtBuff[i] = TubeTab[n % 10];
     n /= 10;     
  }
  TM1637_displayRaw(txtBuff);
